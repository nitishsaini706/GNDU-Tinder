...
router.delete("/:id" ,async(req,res)=>{
    if(req.body.userId == req.params.id || req.body.isAdmin)
    {
        ...
        return res.status(403).json("you can only dalete your account !!!!!");
    }
})
...
router.put("/:id/follow" , async(req,res)=>{
    ...
    {
        try{
            const user = await User.findById(req.params.body);
            const currentUser = await User.findByd(req.params.userId);
